...
def ocr_read(filepath):
    ...
    if os.path.exists("ocr_temp/"):
        shutil.rmtree("ocr_temp/")
    os.makedirs("ocr_temp/")
    ...
    for index in range(num_pages):
        ...
        page.get_pixmap(dpi=200).save(os.path.join("ocr_temp/", temp_img))
    doc.close()

    for index in range(num_pages):
        result = ocr.predict(os.path.join("ocr_temp/", f"temp_{index}.png"))
        print(result)
